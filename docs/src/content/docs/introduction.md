...
If your visualization does not require form rendering, or if you prefer not to use Vue, consider following the [lightweight Vite-only approach](/galaxy-charts/xml-framework#building-a-vite-plugin-vanilla-vue-react-and-more). Both approaches use Vite as the underlying build tool, but only one includes the optional Vue UI package.
